			...
			<?php for($row = 0; $row < intval(count($items) / 4) + 1; $row++) : ?>
				...
				<li class="span3">
		          <div class="thumbnail">
					...
					<img class="product" src="<?php if(count($items_img[$row*4 + $column]) != 0 && !is_null($items_img[$row*4 + $column]['img_address'])) echo base_url().$items_img[$row*4 + $column]['img_address']; else echo 'http://placehold.it/200x100'; ?>" /></a>
		            <div class="caption">
